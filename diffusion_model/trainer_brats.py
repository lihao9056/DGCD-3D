...
def get_named_eta_schedule( #new
        ...
        num_diffusion_timesteps=1000,  #1000
        min_noise_level = 0.001, #0.2
        etas_end=0.99,  #0.9
        kappa=1.0,
        kwargs=None):
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # print(f"self.num:{self.num_timesteps}")
        ...
        # )
        # state_dict = torch.load('./checkpoint/vae/autoencoder_epoch_125.pth')
    ...
    #     # model_output = model(self._scale_input(x_t,t),t,y,y_hat)
    #     #
    #     model_output = self.denoise_fn(x_t,t,y,y_hat)  
        
    #     un = torch.tensor(1.0, dtype=torch.float32)  # 必须转为浮点张量
    ...
    def p_mean_variance(
        self, model, x_t, y, y_hat, un, t,
        clip_denoised=False, denoised_fn=None, model_kwargs=None
    ):
        if model_kwargs is None:
            model_kwargs = {}
    
        v_pred = self.denoise_fn(x_t, t, y, y_hat)
    
        beta_t  = _extract_into_tensor(self.etas, t, x_t.shape)
        alpha_t = 1.0 - beta_t
        sigma_t = _extract_into_tensor(self.sqrt_etas * self.kappa, t, x_t.shape) * un
        y_tilde = y_hat + _extract_into_tensor(self.f, t, x_t.shape) * (y - y_hat)
    
        x_t_center = x_t - beta_t * y_tilde  #y_titde = y (0.23T)
        ...
        model_mean, model_variance, model_log_variance = self.q_posterior_mean_variance(
            x_start=x0_pred, x_t=x_t, y=y, y_hat=y_hat, un=un, t=t
        )
    ...
    def sample(self, batch_size = 1, condition_tensors = None, mask_tensors = None, use_ddim = True, target_step=None):
        if use_ddim == True:
            ...
            with tqdm(indices, desc="DDIM Sampling", unit="step", ncols=100, mininterval=0.1,dynamic_ncols=True,
    leave=False) as pbar:
                for i in pbar:
    ...
    def cal_mse(self, condition_tensor=None, mask=None):
        ...
        un = torch.abs(condition_tensor * (1 - mask))
    
        # un_max = 0.02
        # b_un = 0.2
        
        # un = un.clamp_(0., un_max) / un_max
        # un = b_un + (1 - b_un) * un
    ...
    def p_losses(self, x_start, t, condition_tensors=None, mask=None, noise=None):
        ...
        x_noisy = self.q_sample(
            x_start=x_start, y=condition_tensors, y_hat=mask, un=un, t=t, noise=noise
        )
    
        y_tilde = mask + _extract_into_tensor(self.f, t, x_start.shape) * (condition_tensors - mask)
        beta_t  = _extract_into_tensor(self.etas, t, x_start.shape)            # β_t = η_t
        alpha_t = 1.0 - beta_t                                                 # α_t = 1 - η_t
        sigma_t = _extract_into_tensor(self.sqrt_etas * self.kappa, t, x_start.shape) * un  # σ_t
    
        x_t_center = x_noisy - beta_t * y_tilde
        v_target = alpha_t * noise - sigma_t * x_start   # v = α ε - σ x0
        ...
        for epoch in range(start_epoch, self.num_epochs):
            ...
            for i,data in enumerate(self.dataloader):
                ...
                if self.with_condition:
                    ...
                    # Z_buffer.append(h1.view(1, -1).detach())

                    # if len(Z_buffer) == disp_interval:
                    #     Z_batch = torch.cat(Z_buffer, dim=0)
                    #     Z_batch = F.normalize(Z_batch, p=2, dim=1)
                    #     # print(Z_batch.shape)# shape: [4, D]
                    #     disp = self.model.disp_loss(Z_batch)
                    #     print(f"disp:{disp}")
                    #     total_loss = sum(loss_buffer) / disp_interval
                    #     total_loss += -disp_weight * disp
                    #     total_loss.backward()
                    #     Z_buffer = []
                    #     loss_buffer = []
