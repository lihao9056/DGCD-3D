...
from pyiqa2.arch_util import load_pretrained_network
...
class LPIPS(nn.Module):
    ...
    def __init__(self,
                 ...
                 pnet_tune=False,
                 use_dropout=True,
                 pretrained_model_path='/home/ttyy-user03/daima/un/diffusion_model/pyiqa2/LPIPS_v0.1_vgg-a78928a0.pth',
                 eval_mode=True,
    ...
    def forward(self, in1, in0, retPerLayer=False, normalize=True):
        ...
        if normalize:  # turn on this flag if input is [0,1] so it can be adjusted to [-1, +1] 
            #in0:x_start  in1:ouput


            print(f"--in0:{in0.min()}he{in0.max()}")
            print(f"--in1:{in1.min()}he{in1.max()}")
            # in0 = 2 * in0 - 1
            # in1 = 2 * in1 - 1
        ...
        in0_input, in1_input = (self.scaling_layer(in0), self.scaling_layer(in1)) if self.version == '0.1' else (in0,
                                                                                                                 in1)
        # print(f"ints0:{in0_input.min()}he{in0_input.max()}")
        # print(f"ints1:{in1_input.min()}he{in1_input.max()}")
        outs0, outs1 = self.net.forward(in0_input), self.net.forward(in1_input)
        # print(f"outs:{type(outs0)}")
        # print(f"outs0:{outs0.min()}he{outs0.max()}")
        # print(f"outs1:{outs1.min()}he{outs1.max()}")
        feats0, feats1, diffs = {}, {}, {}
